...
def extract_category_samples(samples: pd.DataFrame, limit: int) -> list[dict]:
    if samples.empty or "value" not in samples.columns:
        return []
    grouped = samples.groupby("value")
    limited: list[dict] = []
    ...
    with rasterio.open(raster_path) as dataset:
        for record in records:
            ...
            if variable.value_type == "categorical":
                ...
                categorical_samples = extract_category_samples(
                    samples,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--variables",
        nargs="+",
        help="Limit processing to one or more GIS variable IDs.",
    )
...
def main() -> None:
    ...
    if args.variables:
        allowed = {var_id.strip() for var_id in args.variables if var_id}
        variables = [variable for variable in variables if variable.id in allowed]
    if not subset_records:
        print("No species matched the provided filters.")
        return
    if not variables:
        print("No GIS variables matched the provided filters.")
        return
