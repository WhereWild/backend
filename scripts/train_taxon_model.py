...
def _configure_storage_modes() -> None:
    ...
    if parquet_mode not in {"local", "b2"}:
        raise ValueError(
            f"Unsupported ml_parquet_storage_mode={CONFIG.ml_parquet_storage_mode!r}; "
            "expected 'local' or 'b2'."
        )
    ...
    if raster_mode not in {"auto", "local", "b2"}:
        raise ValueError(
            f"Unsupported ml_raster_storage_mode={CONFIG.ml_raster_storage_mode!r}; "
            "expected 'auto', 'local', or 'b2'."
        )
...
def _taxon_occurrence_path(taxon_id: str) -> Path:
    ...
    if not storage.exists(path):
        raise FileNotFoundError(f"Occurrence parquet missing for taxon {taxon_id}: {path}")
    return path
...
def _read_positive_features_and_coords(
    ...
    *,
    ...
    if "decimalLatitude" not in available or "decimalLongitude" not in available:
        raise RuntimeError("Positive occurrence parquet missing decimalLatitude/decimalLongitude.")
...
def _sample_dem_derived_values_for_tile(
    ...
    results = {
        metric: np.full(len(lats), np.nan, dtype=np.float32)
...
def _sample_feature_matrix_for_coordinates(
    ...
    values = {
        layer_id: np.full(len(lats), np.nan, dtype=np.float32)
    ...
    regular_layers = [
        layer_id
        for layer_id in feature_columns
        if layer_id != "elevation" and layer_id not in derived_layers
    ]
...
def _prefilter_candidate_coordinates(
    ...
    landcover = pd.to_numeric(mask_features.get("landcover"), errors="coerce").to_numpy(
        dtype=np.float32
    )
    elevation = pd.to_numeric(mask_features.get("elevation"), errors="coerce").to_numpy(
        dtype=np.float32
    )
    ...
    keep_mask = (
        ...
        & ~(
            (landcover_ids == LANDCOVER_WATER_CLASS_ID)
            & (np.abs(elevation) <= SEA_LEVEL_FILTER_ABS_ELEVATION_METERS)
        )
    )
...
def _sample_negative_features_raster(
    ...
    print(
        f"[negatives] planned {len(window_plan)} windows from local rings to global coverage"
    )
    ...
    for idx, window in enumerate(window_plan, start=1):
        ...
        prefilter_target_for_window = max(
            ...
            int(
                np.ceil(
                    target_for_window * float(CONFIG.ml_negative_prefilter_oversample_factor)
                )
            ),
        )
    ...
    rows_collected = 0
    prefilter_prefix = np.concatenate(
        [np.array([0], dtype=np.int64), np.cumsum(prefilter_mask.astype(np.int64))]
    )
...
def _build_negative_window_plan(positive_bbox: BoundingBox) -> list[dict[str, Any]]:
    ...
    if growth_factor <= 1.0:
        raise ValueError(
            f"ml_negative_global_growth_factor must be > 1.0; got {growth_factor!r}"
        )
    ...
    print(
        f"[negatives] final window reaches global={final_bbox.is_global()} "
        f"bbox={final_bbox.to_dict()}"
    )
    return plan
...
def _quantile_summary(values: np.ndarray) -> dict[str, float] | dict[str, Any]:
    ...
    return {
        ...
        "quantiles": {
            f"{q:.2f}": float(np.quantile(finite, q))
    ...
    *,
    ...
    for threshold in EVAL_THRESHOLDS:
        row: dict[str, float] = {
            ...
            "positive_recall": float(np.mean(positive_finite >= threshold))
            if positive_finite.size
            else float("nan"),
        }
        if background_scores is not None:
            row["background_false_positive_rate"] = (
                float(np.mean(background_finite >= threshold))
                if background_finite.size
                else float("nan")
            )
        rows.append(row)
    return rows
...
def main() -> None:
    ...
    if CONFIG.ml_phenology_mode:
        ...
        print(
            f"[train] mode=phenology (rcs annotation as label, no synthetic negatives) "
            f"temporal_only={temporal_only}"
        )
        ...
        if temporal_only:
            if not all_temporal_cols:
                raise RuntimeError(
                    "ml_phenology_temporal_only=True but no temporal columns found in parquet."
                )
        ...
        _run_training(
            taxon_id, positives, negatives, full_feature_spec, negative_info,
            mode="sdm", positive_bbox=positive_bbox,
        )
