...
PRECIP_COL = "precipitation_sum_24h"
...
def _fetch_openmeteo_precip_24h(lat: float, lon: float, dt: datetime) -> tuple[float, str]:
    ...
    if not times or not vals:
        pytest.skip("Open-Meteo API returned no hourly precipitation")
    ...
    if idx_start < 0:
        pytest.skip("Not enough hourly data to compute 24h sum")

    window = vals[idx_start:idx_end + 1]
    if len(window) != 24:
        pytest.skip("Unexpected hourly window size for 24h sum")
...
def test_precipitation_matches_openmeteo(row_index: int) -> None:
    if not PARQUET_PATH.exists():
        pytest.skip("Occurrence parquet not found")

    table = pq.read_table(PARQUET_PATH, columns=[LAT_COL, LON_COL, TIME_COL, PRECIP_COL]).combine_chunks()
    ...
    val = row[PRECIP_COL][0].as_py()

    if lat is None or lon is None or ts is None:
        pytest.skip("Row missing lat/lon/timestamp")
    if val is None or (isinstance(val, float) and np.isnan(val)):
        pytest.skip("Row missing precipitation value")

    dt = _to_datetime_utc(ts)
    api_val, url = _fetch_openmeteo_precip_24h(float(lat), float(lon), dt)

    print(
        f"row={row_index} lat={lat:.6f} lon={lon:.6f} time={dt.isoformat()} "
        f"parquet={float(val)} api={api_val} url={url}"
    )

    # Tight tolerance for direct API comparison
    assert abs(float(val) - api_val) <= 0.1


@pytest.mark.parametrize("row_index", [0, 1, 2])
def test_precipitation_omfiles_matches_openmeteo(row_index: int) -> None:
    if not PARQUET_PATH.exists():
        pytest.skip("Occurrence parquet not found")
    ...
    print(
        f"row={row_index} lat={lat:.6f} lon={lon:.6f} time={dt.isoformat()} "
        f"omfiles={om_val} api={api_val} url={url}"
    )
...
@pytest.mark.parametrize("row_index", [0, 1, 2])
def test_precipitation_parquet_matches_omfiles(row_index: int) -> None:
    if not PARQUET_PATH.exists():
        pytest.skip("Occurrence parquet not found")
    ...
    print(
        f"row={row_index} lat={lat:.6f} lon={lon:.6f} time={dt.isoformat()} "
        f"parquet={float(val)} omfiles={om_val}"
    )
